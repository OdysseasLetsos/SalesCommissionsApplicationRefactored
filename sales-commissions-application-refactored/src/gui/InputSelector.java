...
public class InputSelector extends JDialog {
	...
	static InputSelector dialog = new InputSelector();
	@SuppressWarnings("unused")
	...
	private void insertFromXML(ActionEvent evt2){
		...
		try{
			...
			salesman = inputFileXML.getAgent();
	...
	private void insertFromHTML(ActionEvent evt3){
		...
		try{
			...
			salesman=inputFileHTML.getAgent();
			//TO DO fileAppender for HTML
	...
	private void okButtonPressed(ActionEvent evt) {
		if(salesmenList.isSelectionEmpty()){
			...
			SelectionChooser sw = new SelectionChooser(dialog,selectedSalesman,fileTypeFlag);
