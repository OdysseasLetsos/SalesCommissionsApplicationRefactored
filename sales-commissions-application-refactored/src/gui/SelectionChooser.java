...
public class SelectionChooser extends JDialog {
	...
	private float shirtSales=-1;
	private float skirtSales=-1;
	private float coatsSales=-1;
	private float trousersSales=-1;
	private double commission=-1;
	private InputSelector inputDialog;
	private Salesman selectedSalesman;
	@SuppressWarnings("unused")
	private String fileType;
	//private ReceiptType receipttype;
	
	public SelectionChooser(InputSelector dialog, Salesman agent, String fileTypeFlag) {
		inputDialog = dialog;
		selectedSalesman = agent;
	...
	public void initialise(){
		...
		try{
			...
			JOptionPane.showMessageDialog(null,"Προέκυψε κάποιο πρόβλημα, προσπαθήστε ξανά11111111");
	...
	protected void okButtonPressed(ActionEvent evt) {
		...
		/*if(checkBoxSelection()){
			makeTheCalculations()update
		}*/
		ReportViewer rs = new ReportViewer(this,selectedSalesman, totalSales, totalItems, shirtSales, skirtSales, trousersSales, coatsSales, commission);
		rs.setVisible(true);
		this.setVisible(false);		
	}
	/*private boolean checkBoxSelection(ArrayList<Object> checkBoxArrayList) {
		for (Object jCheckBox : checkBoxArrayList) {
			if(((AbstractButton) jCheckBox).isSelected()) {
					
			}		
			return false;
	}*/
	
	/*private ArrayList<Object> createJCheckBoxs() {
		ArrayList<Object> checkBoxArrayList=new ArrayList<>();
		checkBoxArrayList.add(totalSalesCheckBox);
		checkBoxArrayList.add(totalItemsCheckBox);
		checkBoxArrayList.add(commissionCheckBox);
		checkBoxArrayList.add(skirtRadio);
		checkBoxArrayList.add(shirtRadio);
		checkBoxArrayList.add(coatRadio);
		checkBoxArrayList.add(trousersRadio);
		return checkBoxArrayList;
	}*/
	
	/*private ArrayList<Object> createOurvaluesForEachBox(){
		ArrayList<Object> totalvaluesArrayList=new ArrayList<>();
		totalvaluesArrayList.add(totalSales);
		totalvaluesArrayList.add(totalItems);
		totalvaluesArrayList.add(commission);
		return totalvaluesArrayList;
	}*/
	...
	//TODO Refactor is DECOMPOSE CONDITIONAL
	private void addReceiptButtonPressed(ActionEvent evt) {
		if(checkTheTextFields(createFieldsArrayList())){
			System.out.println("δεν εχει συμπληρωσει τα πεδια");
			JOptionPane.showMessageDialog(null,"Πρέπει να συμπληρώσετε όλα τα πεδία");
			/*receiptIDTextField.getText().isEmpty() || dateTextField.getText().isEmpty() 
	...
	private boolean checkTheTextFields(ArrayList<JTextField> fielsArrayList) {
		for (JTextField jTextField : fielsArrayList) {
	...
	private ArrayList<JTextField> createFieldsArrayList(){
		ArrayList<JTextField> fieldsArrayList=new ArrayList<>();
		fieldsArrayList.add(receiptIDTextField);
		fieldsArrayList.add(dateTextField);
		fieldsArrayList.add(kindTextField);
		fieldsArrayList.add(salesTextField);
		fieldsArrayList.add(itemsTextField);
		fieldsArrayList.add(companyTextField);
		fieldsArrayList.add(countryTextField);
		fieldsArrayList.add(cityTextField);
		fieldsArrayList.add(streetTextField);
		fieldsArrayList.add(numberTextField);
	...
	private void appendFile(){
		...
		receipt.setCompanyName(companyTextField.getText());
		//receipt.getCompany().getCompanyAddress().setCountry(countryTextField.getText());
		receipt.setCompanyCountry(countryTextField.getText());
		//receipt.getCompany().getCompanyAddress().setCity(cityTextField.getText());
		receipt.setCompanyCity(cityTextField.getText());
		//receipt.getCompany().getCompanyAddress().setStreet(streetTextField.getText());
		receipt.setCompanyStreet(streetTextField.getText());
		//receipt.getCompany().getCompanyAddress().setStreetNumber(Integer.parseInt(numberTextField.getText()));
		receipt.setCompanyNumber(numberTextField.getText());
		
		selectedSalesman.getFileAppender().modifyReceipt(receipt);
		//gia auton ton salesman sto sygkekrimeno fileappender tha enhmervsei tiw times se string kai
	...
	private void addReceipt(){
		...
		try{
			//edw sto antikeimeno receipt gia auto ta metatrepei se int
			receipt.setReceiptID(Integer.parseInt(receiptIDTextField.getText()));	
			receipt.setKind(kindTextField.getText());
			receipt.setDate(dateTextField.getText());
			receipt.setSales(Double.parseDouble(salesTextField.getText()));
