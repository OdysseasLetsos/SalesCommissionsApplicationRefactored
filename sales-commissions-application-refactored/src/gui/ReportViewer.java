...
import output.HTMLReportSaver;
import output.TXTReportSaver;
import output.XMLReportSaver;
...
public class ReportViewer extends JDialog {
	...
	private SelectionChooser selectionChooser;
	...
	public ReportViewer(final SelectionChooser sw, Salesman salesman,double tSales,int tItems,
	...
	private void initialise(){
		...
		resultWindowPanel.setLayout(null);
		{
			...
			xmlReportButton.setBounds(436, 32, 163, 84);
		...
		}
		{
			...
			txtReportButton.setBounds(436, 150, 163, 81);
			resultWindowPanel.add(txtReportButton);
		}
		//TO DO this for the export ButtonGroup html
		{
			...
			htmlReportButton.setBounds(436, 255, 163, 84);
	...
	private void updateResults() {
		...
		if(trousersSales>=0)
			...
			trouserSalesTextField.setEnabled(false);
		if(commission>=0)
	...
	private void outputTXTButtonPressed(ActionEvent evt) {
		...
		TXTReportSaver makeTXTFile = new TXTReportSaver(selectedSalesman);
		...
		File myFileChooser=jfc.getSelectedFile();
		makeTXTFile.saveFile(myFileChooser.getAbsolutePath());
	...
	private void outputXMLButtonPressed(ActionEvent evt) {
		...
		jfc.setDialogTitle("Choose a directory to save your file: ");
		jfc.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
		XMLReportSaver makeXMLFile = new XMLReportSaver(selectedSalesman);
		int returnValue = jfc.showSaveDialog(null);
		if (returnValue == JFileChooser.APPROVE_OPTION) {
			if (jfc.getSelectedFile().isDirectory()) {
				System.out.println("You selected the directory: " + jfc.getSelectedFile());
		...
		File myFileChooser=jfc.getSelectedFile();
		makeXMLFile.saveFile(myFileChooser.getAbsolutePath());
		JOptionPane.showMessageDialog(null,"Η αναφορά αποθηκεύτηκε επιτυχώς");		
	}
	//TO DO this for outputHTML
	private void outputHTMLButtonPressed(ActionEvent evt) {
		JFileChooser jfc = new JFileChooser(FileSystemView.getFileSystemView().getHomeDirectory());
		jfc.setDialogTitle("Choose a directory to save your file: ");
		jfc.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
		HTMLReportSaver makeHTMLFile = new HTMLReportSaver(selectedSalesman);
		int returnValue = jfc.showSaveDialog(null);
		if (returnValue == JFileChooser.APPROVE_OPTION) {
			if (jfc.getSelectedFile().isDirectory()) {
				System.out.println("You selected the directory: " + jfc.getSelectedFile());
		...
		File myFileChooser=jfc.getSelectedFile();
		makeHTMLFile.saveFile(myFileChooser.getAbsolutePath());
		JOptionPane.showMessageDialog(null,"Η αναφορά αποθηκεύτηκε επιτυχώς");		
	}
